...
class _KaganVideoScreenState extends State<KaganVideoScreen> {
  ...
  Future<void> _fetchVideos() async {
    try {
      ...
      debugPrint('Attempting URL: ${_baseUrl}category=video&tribe=kagan');

      // Try multiple approaches to get data
      List<http.Response> responses = [];
      List<String> urls = [
        '${_baseUrl}category=video&tribe=kagan',  // Original attempt
        '$_baseUrl?category=video&tribe=kagan', // With query parameter format
        '$_baseUrl?tribe=kagan',                // Just tribe filter
      ...
      for (var item in videoData) {
        ...
        // 2. Must be Kagan tribe
        if (tribe.toLowerCase() != 'kagan') {
          debugPrint('Skipping item - not Kagan tribe: $tribe');
          continue;
        }
        ...
        // 5. Optional: Uncomment to only include video files
        // if (fileType != FileType.video) {
        //   debugPrint('Skipping item - not a video file: $file (type: $fileType)');
        //   continue;
        // }
      ...
      if (fetchedVideos.isEmpty) {
        debugPrint('No videos found - this might be expected if there are no Kagan videos in the database');
      }
  ...
  Widget _buildDebugInfo() {
    if (!_isLoading && _allVideos.isEmpty && _errorMessage == null) {
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Expected data for Kagan tribe with video category',
              style: TextStyle(
                color: Colors.white.withOpacity(0.7),
                fontSize: 12,
              ),
            ...
            Text(
              'Total items loaded: ${_allVideos.length}',
              style: TextStyle(
                color: Colors.white.withOpacity(0.7),
                fontSize: 12,
              ),
